function [gbestval,gbest,run_funcvals]= TIS_LSHADE_SPACMA(N,FESmax,lb,ub,dim,fobj)
L_Rate= 0.80;
lu = [lb; ub];
p_best_rate = 0.11;
arc_rate = 1.4;
...
% lu=repmat(lu,1,dim);
fitness(1,1)=feval(fobj,pop(1,:));
FES=1;
m=1;
person(m,:)=pop(1,:);             %store the first memory
personf(m,1)=fitness(1,1); 

for i = 1 : pop_size
    [m,person,personf,popold(i,:),fitness(i,:),FES]=...
            TIS(pmax,m,person,personf,popold(i,:),fobj,FES,lu,FESmax);

    if (fitness(i) < gbestval && isreal(pop(i, :)) && sum(isnan(pop(i, :)))==0 )
        gbestval = fitness(i);
        gbest = popold(i, :);
    end
end

run_funcvals = ones(1,FES-length(run_funcvals))*gbestval;
...
while 1
    ...
    children_fitness=[];
    for i = 1 : pop_size
        [m,person,personf,ui(i,:),children_fitness(i,:),FES]=...
            TIS(pmax,m,person,personf,ui(i,:),fobj,FES,lu,FESmax);

        if (children_fitness(i) < gbestval && isreal(ui(i, :)) && sum(isnan(ui(i, :)))==0)
            gbestval = children_fitness(i);
            gbest = ui(i, :);
        end
        if FES >= FESmax
            run_funcvals = [run_funcvals,ones(1,FES-length(run_funcvals))*gbestval];
            return
        end
    end
    ...
    plan_pop_size = round((((min_pop_size - max_pop_size) / FESmax) * FES) + max_pop_size);
